...
import { ArrowLeft, AlertCircle, RefreshCw, Hash, CalendarDays, ExternalLink, Trash2 } from 'lucide-react';
...
function getLinked(payload: any) {
  const p = payload || {};
  return {
    poId: p.source_po_id || p.po_id || null,
    poNo: p.source_po_no || p.po_no || null,
    srId: p.source_sr_id || p.sr_id || null,
...
export default function GRNDetail() {
  ...
  const fetchOne = async () => {
    try {
      ...
      console.error(e);
      setError(e?.message || 'Failed to load GRN');
      setRow(null);
    } finally {
      setLoading(false);
    }
  ...
  const deleteDraft = async () => {
    ...
    try {
      ...
      console.error(e);
      setError(e?.message || 'Failed to delete GRN');
    } finally {
      setDeleting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-100">
        <div className="max-w-5xl mx-auto py-10 px-4">
          <button onClick={() => navigate('/procurement/grn')} className="text-sm text-gray-700 underline" type="button">
            Back to list
          </button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-bold text-gray-900">GRN Details</h1>
            </div>
